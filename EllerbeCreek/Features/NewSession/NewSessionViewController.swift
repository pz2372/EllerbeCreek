...
class NewSessionViewController: UIViewController {
    ...
    private let newSessionView: NewSessionView

    // MARK - Variables

    // MARK: - UIViewController Lifecycle

    required init(navigator: NewSessionNavigator, preserve: Preserve) {
        self.navigator = navigator
        self.storage = navigator.dependencyContainer.storage
        self.newSessionView = NewSessionView(preserve: preserve)
